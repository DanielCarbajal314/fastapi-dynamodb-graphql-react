...
export function UseProjectState() {
    ...
    const { loading, data: projectData } = useQuery<ProjectResult>(GET_PROJECTS);
    const projects = projectData?.projects ?? [];
    const [ createUserMutation, { loading: createUserLoading }] = useMutation<CreateProjectResult>(CREATE_PROJECTS, {
        refetchQueries: [{ query: GET_PROJECTS }]
    });
    const createUser = (name: string) => createUserMutation({ variables: { name } });
